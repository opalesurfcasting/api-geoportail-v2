 ...
 */
/*
 * @requires Geoportal/Layer/Grid.js
 */
/**
 * Class: Geoportal.Layer.WMTS
 * Instances of the Geoportal.Layer.WMTS class allow viewing of tiles from a service that
 *     implements the OGC WMTS specification version 1.0.0.
 *
 * Inherits from:
 *  - {<Geoportal.Layer.Grid>}
 */
Geoportal.Layer.WMTS = OpenLayers.Class( Geoportal.Layer.Grid, {

    /**
     * APIProperty: isBaseLayer
     * {Boolean} The layer will be considered a base layer.  Default is false.
     */
    isBaseLayer: false,
     ...
     * {<OpenLayers.LonLat at http://dev.openlayers.org/releases/OpenLayers-${openlayers.version}/doc/apidocs/files/OpenLayers/LonLat-js.html>} The top-left corner of the tile matrix in map
     ...
     * {<OpenLayers.Bounds at http://dev.openlayers.org/releases/OpenLayers-${openlayers.version}/doc/apidocs/files/OpenLayers/Bounds-js.html>}  The full extent of the tile set.  If not supplied,
     ...
     * topLeftCorner - {<OpenLayers.LonLat at http://dev.openlayers.org/releases/OpenLayers-${openlayers.version}/doc/apidocs/files/OpenLayers/LonLat-js.html>} The top left corner of the
    ...
    /**
     * Constructor: Geoportal.Layer.WMTS
     * Create a new WMTS layer.
     *
     * Example:
     * (code)
     * var wmts = new Geoportal.Layer.WMTS({
     *     name: "My WMTS Layer",
    ...
    initialize: function(config) {
        ...
        // Old way of invoking constructor (name, url, params, options) :
        if (arguments.length==4) {
            var cnf= OpenLayers.Util.extend({}, arguments[2]);
            OpenLayers.Util.extend(cnf, arguments[3]);
            OpenLayers.Util.extend(cnf, {
                name:arguments[0],
                url :arguments[1]
            });
            config= cnf;
        ...
        for (var prop in required) {
            if (!(prop in config)) {
                throw new Error("Missing property '" + prop + "' in layer configuration.");//TODO i18n
            }
        }

        config.params = OpenLayers.Util.upperCaseObject(config.params);
        var args = [config.name, config.url, config.params, config];
        Geoportal.Layer.Grid.prototype.initialize.apply(this, args);
     ...
     */
    setMap: function() {
        Geoportal.Layer.Grid.prototype.setMap.apply(this, arguments);
    ...
    updateMatrixProperties: function() {
        this.getServerResolution(); // force update this.nativeResolution value
        this.matrix = this.getMatrix();
        if (this.matrix) {
            if (this.matrix.topLeftCorner) {
                this.nativeTileOrigin= this.matrix.topLeftCorner;
            ...
            if (!this.tileFullExtent || this.resample) {
                this.tileFullExtent= this.resample? this.nativeMaxExtent : this.maxExtent;
            }
        }
    },
     ...
     *
     * Parameters:
     * bound - {<OpenLayers.Bounds at http://dev.openlayers.org/releases/OpenLayers-${openlayers.version}/doc/apidocs/files/OpenLayers/Bounds-js.html>}
     ...
     *
     * Returns:
     * {<OpenLayers.Layer.WMTS at http://dev.openlayers.org/releases/OpenLayers-${openlayers.version}/doc/apidocs/files/OpenLayers/Layer/WMTS-js.html>} An exact clone of this
     *  <Geoportal/Layer/WMTS>
     */
    clone: function(obj) {
        if (obj == null) {
            obj = new Geoportal.Layer.WMTS(this.options);
        }
    ...
    getTileInfo: function(loc) {
        var res, w, h, toLon, toLat;
        if (this.nativeResolution) {
            res= this.nativeResolution;
            w= this.nativeTileSize.w;
            h= this.nativeTileSize.h;
            toLon= this.nativeTileOrigin.lon;
            toLat= this.nativeTileOrigin.lat;
        } else {
            res= this.map.getResolution();
            w= this.tileSize.w;
            h= this.tileSize.h;
            toLon= this.tileOrigin.lon;
            toLat= this.tileOrigin.lat;
        }

        var fx= (loc.lon - toLon)   / (res * w);
        var fy= (toLat   - loc.lat) / (res * h);
        ...
        return {
            col: col,
            row: row,
            i: Math.floor((fx - col) * w),
            j: Math.floor((fy - row) * h)
        };
    },
    ...
    getURL: function(bounds) {
        ...
        if (!this.tileFullExtent || this.tileFullExtent.intersectsBounds(bounds)) {
            ...
            if (this.requestEncoding.toUpperCase() === "REST") {
                ...
                url = Geoportal.Layer.Grid.prototype.getFullRequestString.apply(this, [params]);
    ...
    mergeNewParams: function(newParams) {
        if (this.requestEncoding.toUpperCase() === "KVP") {
            return Geoportal.Layer.Grid.prototype.mergeNewParams.apply(
     ...
     *  Modify transitionEffect whenever opacity is changed to
    ...
    setOpacity: function(opacity) {
        // when transitionEffect is defined
        if (this.transitionEffect!=null) {
            if (opacity<1.0) {
                this.transitionEffect= '!resize';//FAKE value
            } else {
                this.transitionEffect= 'resize';//FIXME: only supported value ?
            }
        }
        Geoportal.Layer.Grid.prototype.setOpacity.apply(this,[opacity]);
    },

    /**
     * Constant: CLASS_NAME
     * {String} *"Geoportal.Layer.WMTS"*
     */
